...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('socialmedia.store')}}" method="post" class="needs-validation" novalidate="" autocomplete="off">
                @csrf
                <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" dir="rtl" name="type">
                                <option selected value="0">انتخاب کنید</option>
                                        @if(!in_array(1,$types)) @else <option value="1">اینستاگرام</option> @endif
                                        @if(!in_array(2,$types)) @else <option value="2">لینکدین</option> @endif
                                        @if(!in_array(3,$types)) @else <option value="3">توییتر</option> @endif
                                        @if(!in_array(4,$types)) @else <option value="4">تلگرام</option> @endif
                                        @if(!in_array(5,$types)) @else <option value="5">یوتیوب</option> @endif
                                        @if(!in_array(6,$types)) @else <option value="6">گپ</option> @endif
                                        @if(!in_array(7,$types)) @else <option value="7">سروش</option> @endif
                                        @if(!in_array(8,$types)) @else <option value="8">آپارات</option> @endif
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <td>
                            <div class="form-group">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" data-id="{{$item->id}}" class="custom-control-input" id="customSwitch_{{$key}}" @if($item->status == 1) checked @endif>
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                ...
                                <div class="dropdown-menu" aria-labelledby="btnGroupDrop2">
                                    <a class="dropdown-item editItem" data-id="{{$item->id}}"><i class="fa fa-pencil ml-3"></i>ویرایش</a>
