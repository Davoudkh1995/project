...
@section('content')
            ...
            <form action="{{route('service.update',$service->id)}}" method="post" class="needs-validation" novalidate=""
                ...
                <div class="form-row">
                    <div class="row form-group col-md-12">
                        ...
                        <div class="col-md-4 text-right">
                            <a href="{{$service->picture['main']}}"><img src="{{$service->picture['main']}}" alt="" width="100" height="50"></a>
                        </div>
                    </div>
                    @foreach($otherImages as $key=>$image)
                ...
                <div class="form-row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="validationCustom01" required="" name="title" value="{{$service->title}}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" dir="rtl" name="categoryID_FK">
                                ...
                                @foreach($categories as $item)
                                    <option value="{{$item->id}}" @if($service->categoryID_FK == $item->id) selected @endif>{{$item->title}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-row">
                    <div class="col-md-4 mb-3">
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" dir="rtl" name="lang">
                                <option @if($service->lang == "fa") selected @endif value="fa">فارسی</option>
                                <option @if($service->lang == "en") selected @endif value="en">انگلیسی</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-control js-example-basic-single" dir="rtl" name="priority" id="priority">
                            ...
                            <option disabled selected>{{$service->priority}}</option>

                            @foreach($priorities as $priority)
                                <option value="{{$priority}}" @if($service->priority == $priority) selected @endif>{{$priority}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-row mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea rows="5" id="body" cols="5" class="form-control"
                                  name="content"
                                  placeholder="محتوای صفحه خود را در این قسمت وارد کنید...">{{$service->content}}</textarea>
                    </div>
                    <script>
                ...
                <div class="form-row form-group">
                    <div class="custom-control custom-checkbox custom-checkbox-success">
                        <input type="checkbox" class="custom-control-input" id="customCheck" @if($service->status == 1) checked @endif name="status">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('save_seo_service')}}" method="post">
                @csrf
                @method('patch')
                <input type="hidden" name="object" value="{{$service->id}}">
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
