        ...
        <div class="card-body">
            <div class="btn-group" role="group">
                ...
                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                    <a class="dropdown-item" style="cursor: pointer" href="/admin/permission/create">ثبت
                        اطلاعات</a>
                        ...
                        {{--<td class="idCheckbox">
                            <div class="custom-control custom-checkbox custom-checkbox-primary">
                                <input type="checkbox" class="custom-control-input" value="{{$permission->id}}"
                                       onclick="remove_all(this);" id="customCheck_{{$permission->id}}">
                                <label class="custom-control-label" for="customCheck_{{$permission->id}}"></label>
                            </div>
                        </td>--}}
                        <th scope="row"><?= ++$i ?></th>
                        <td>{{$permission->name}}</td>
                        <td>{{$permission->label}}</td>
                        <td><a href="{{route('permission.edit',['id'=>$permission->id])}}"><i
                                        class="fa fa-edit font-size-23"
                                       ></i></a></td>
                        {{--<td>
                            <form action="{{route('permission.destroy',['id'=>$permission->id])}}" method="post" id="remove">
                                {{csrf_field()  }}
                                {{method_field('delete')}}
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            ...
                            $.ajax({
                                ...
                                type: "post",
                                cache: false,
                                url: "{{route('multiRemovePermission')}}",
                    ...
                    if (data['delete'] == 'success') {
                        ...
                        $.each(permissions, function (key, value) {
                                ...
                                var itemMultiRemove = ' <tr><td class="idCheckbox"><div class="checker bg-white"><span class="' + spanCheck + '"><input class="check styled" type="checkbox" name="id" value="' + value['id'] + '" onclick="any(this);" ' + inputCheck + '></span></div></td>'
                                    ...
                                    + '<td></a>' + value['name'] + '</td>'
                                    + '<td></a>' + value['label'] + '</td>'
                                    + '<td><a href="/admin/permission/' + value["id"] + '/edit"><i class="glyphicon glyphicon-edit" style="color: #000;width:15px;height: 19px"></i></a></td>'
...
//            console.log(allCheckedSelect);
            ...
            $.ajax({
                ...
                type: "post",
                cache: false,
                {{--                url: "{{route('searchTitlePermission')}}",--}}
                data: {'_token': '{{csrf_token()}}', value: value},
                success: function (data) {
                    var custom = data['permissions'];
                    var counter1 = 1;
                    tbody1.html('');
                    var spanCheck = '';
                    var inputCheck = '';
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        type: "post",
                        cache: false,
                        {{--                        url: "{{route('singleRemovePermission')}}",--}}
