...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('category_portfolio.update',$categoryPortfolio->id)}}" method="post" class="needs-validation" novalidate=""
                  autocomplete="off">
                @csrf
                @method('patch')
                <div class="form-row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="validationCustom01" required="" name="title" value="{{$categoryPortfolio->title}}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="validationCustom04" placeholder="نامک" required=""
                               name="slug" value="{{$categoryPortfolio->slug}}">
                        <div class="valid-feedback">
                            صحیح است!
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" dir="rtl" name="parent_id">
                                ...
                                @foreach($category_portfolios as $item)
                                    @if($categoryPortfolio->id != $item->id)
                                    <option value="{{$item->id}}" @if($categoryPortfolio->parent_id == $item->id) selected @endif>{{$item->title}}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" dir="rtl" name="lang">
                                <option @if($categoryPortfolio->lang == "fa") selected @endif value="fa">فارسی</option>
                                <option @if($categoryPortfolio->lang == "en") selected @endif value="en">انگلیسی</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="validationCustom02">برچسب</label>
                        <input type="text" class="form-control tagsinput" value="{{$categoryPortfolio->tags}}" name="tags">
                        <div class="valid-feedback">
                            صحیح است!
                        </div>
                    </div>
                ...
                <div class="form-row form-group">
                    <div class="custom-control custom-checkbox custom-checkbox-success">
                        <input type="checkbox" class="custom-control-input" id="customCheck" @if($categoryPortfolio->status == 1) checked @endif name="status">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('save_seo_portfolio_cat')}}" method="post">
                @csrf
                @method('patch')
                <input type="hidden" name="object" value="{{$categoryPortfolio->id}}">
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
